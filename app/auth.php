...
function base32_decode_custom(string $secret): string
{
    ...
    foreach (str_split($binary, 8) as $byte) {
        if (strlen($byte) === 8) $data .= chr(bindec($byte));
    }
    return $data;
}
...
{
    $dark = ($_SESSION['theme'] ?? 'light') === 'dark';
    $bodyClass = $dark ? 'bg-dark text-light' : 'bg-light';
    ...
    echo '<link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet">' . $cardCss;
    echo '</head><body class="' . $bodyClass . '"><nav class="navbar navbar-expand-lg navbar-dark bg-dark mb-4"><div class="container">';
    echo '<a class="navbar-brand" href="index.php">3D Print Tracker</a>';
    if (is_logged_in()) {
        echo '<div class="navbar-nav"><a class="nav-link" href="filament_add.php">Filament</a><a class="nav-link" href="job_add.php">Druckjob</a><a class="nav-link" href="maintenance.php">Wartung</a><a class="nav-link" href="2fa_setup.php">2FA</a>';
        if (is_admin()) echo '<a class="nav-link" href="admin_users.php">User</a>';
        echo '</div>';
        echo '<span class="navbar-text ms-auto me-3">' . e(current_user_name()) . '</span><a class="btn btn-outline-warning btn-sm me-2" href="toggle_theme.php">Theme</a><a class="btn btn-outline-light btn-sm" href="logout.php">Logout</a>';
    }
